...
public class HairInstance : MonoBehaviour
{
	...
	public string m_shadow_shader = "HairWorksIntegration/DefaultShadowShader.cso";
	...
	hwHShader m_shadowShader = hwHShader.NullHandle;
	...
	public RenderTexture HairWorksShadowMap;
	...
	public void LoadHairShader(string path_to_cso, bool isShadowShader)
    {
		if (!isShadowShader) {
			...
			if (m_shadowShader) {
				HairWorksIntegration.hwShaderRelease (m_shadowShader);
				m_shadowShader = hwHShader.NullHandle;
			}
			if (m_shadowShader = HairWorksIntegration.hwShaderLoadFromFile (Application.streamingAssetsPath + "/" + path_to_cso)) {
				m_shadow_shader = path_to_cso;
			}
		}
#if UNITY_EDITOR
        RepaintWindow();
	...
	{
		HairWorksIntegration.hwShaderReload (m_shadowShader);
		RepaintWindow();
	}
  ...
  public  void Reset()
    ...
    void Start()
    {
        LoadHairShader(m_hair_shader, false);
		LoadHairShader (m_shadow_shader, true);
        LoadHairAsset(m_hair_asset, false);
		lights = FindObjectsOfType (typeof(HairLight)) as HairLight[];
		updateTextures = true;
		colorBuffer = Graphics.activeColorBuffer;
		depthBuffer = Graphics.activeDepthBuffer;
		HairWorksShadowMap = new RenderTexture (2048, 2048, 16, RenderTextureFormat.RFloat, RenderTextureReadWrite.Linear);
		HairWorksShadowMap.generateMips = false;
		HairWorksShadowMap.useMipMap = false;
		HairWorksShadowMap.anisoLevel = 0;
		HairWorksShadowMap.filterMode = FilterMode.Point;
		HairWorksShadowMap.wrapMode = TextureWrapMode.Clamp;
		HairWorksShadowMap.Create ();
    ...
    static void BeginRender()
    {
		...
		bool goodToGo = !cam.CompareTag("ShadowCamera");
        if(cam != null && goodToGo)
        {

            Matrix4x4 V = cam.worldToCameraMatrix;
            Matrix4x4 P = GL.GetGPUProjectionMatrix(cam.projectionMatrix, DoesRenderToTexture(cam));
			HairLight hLights = FindObjectOfType (typeof(HairLight)) as HairLight;
            float fov = cam.fieldOfView;
			Matrix4x4 S = hLights.m_shadowSpaceMatrix * hLights.shadowProjMat;
	...
	void BeginRenderShadow(){
		if (s_command_buffer != null) {
			foreach (HairLight hLight in lights) {
				if (hLight.castsShadows) {
					hLight.RenderShadowDepth ();
   ...
   public void Render()
    {
		...
		HairWorksIntegration.hwRenderShadow (m_hinstance, lights [0].shadowTexture.GetNativeTexturePtr ());
