...
hwCLinkage hwExport void hwInstanceSetTexture(hwHInstance iid, hwTextureType type, ID3D11Texture2D* tex)
{
    if (auto ctx = hwGetContext()) {
...
hwCLinkage hwExport void hwRenderShadow(hwHInstance iid, ID3D11Texture2D* shadowMap/*, int numShadowCasters*/)
{
    if (auto ctx = hwGetContext()) {
        ctx->renderShadow(iid, shadowMap/*, numShadowCasters*/);
    }
}
...
hwCLinkage hwExport void hwSetSphericalHarmonics(const hwFloat4 *Ar, const hwFloat4 *Ag, const hwFloat4 *Ab, const hwFloat4 *Br, const hwFloat4 *Bg, const hwFloat4 *Bb, const hwFloat4 *C)
{
	if (auto ctx = hwGetContext()) {
...
hwCLinkage hwExport void hwInitShadows(int resolution, ID3D11Texture2D *shadowMap){
	if (auto ctx = hwGetContext()) {
		ctx->initShadows(resolution, shadowMap);
	}
}

hwCLinkage hwExport void  hwPrepareRenderTarget(){
	if (auto ctx = hwGetContext()) {
		ctx->prepareRenderTarget();
	}
}

hwCLinkage hwExport void hwPrepareShadowTarget(){
	if (auto ctx = hwGetContext()) {
		ctx->prepareShadowTarget();
	}
}
